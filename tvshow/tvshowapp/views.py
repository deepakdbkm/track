...
def home(request):
    if not request.user.is_authenticated:
        demo_videos = VideoPost.objects.all().order_by('-id')[:6]
...
def signup(request):
    if request.method == 'POST':
        ...
        new_user = User.objects.create_user(first_name=first_name,last_name=last_name,username=uname,email=mail,password=pwd)
